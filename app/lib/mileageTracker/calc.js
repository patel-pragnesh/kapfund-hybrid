...
function calculateDistanceBetweenPoints(pointA, pointB) {
	...
	// Math.PI / 180
	var c = Math.cos;
	var a = 0.5 - c((pointB.latitude - pointA.latitude) * p) / 2 + c(pointA.latitude * p) * c(pointB.latitude * p) * (1 - c((pointB.longitude - pointA.longitude) * p)) / 2;

	return 12799188 * Math.asin(Math.sqrt(a));
	// 2 * R; R = 6399594 m
}

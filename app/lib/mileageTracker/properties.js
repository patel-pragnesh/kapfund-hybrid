...
function set(key, val) {
	...
	if (!App.CFG.settings[key]) {
		throw new Error('Unknown setting: ' + key);
	}

	return _set(App.CFG.settings[key].type, key, val);
}
...
function get(key, def) {
	...
	return _get(App.CFG.settings[key].type, key, (arguments.length === 2) ? val : App.CFG.settings[key].def);
}
